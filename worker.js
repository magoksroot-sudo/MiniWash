  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .badge {
      ...
      border-radius: 999px;
      background: linear-gradient(180deg, rgba(85,107,47,0.12), rgba(85,107,47,0.04));
      border: 1px solid rgba(85,107,47,0.09);
      box-shadow: 0 6px 16px rgba(85,107,47,0.06);
      margin-top: .25rem;
    }

    /* SVG icon size */
    .icon svg{ width: clamp(36px, 8vw, 56px); height:auto; display:block; }
    ...
    /* Accessibility: visually-hidden for screen readers if needed */
    .sr-only{
    ...
    @media (min-width:900px){
      body{padding:2.5rem}
      .card{padding: clamp(2rem, 2.6vw, 3.5rem); border-radius: calc(var(--radius) + 1rem); width: min(88%, var(--max-width));}
      .brand{font-size:1.15rem}
      .countdown{min-width: 260px}
    }

    /* Respect reduced motion */
    @media (prefers-reduced-motion: no-preference){
      .card{transform: translateY(0); transition: transform .45s cubic-bezier(.2,.9,.3,1);}
      body.loaded .card{ transform: translateY(-4px); }
    }

    /* Small adjustments for very small screens */
    @media (max-width:360px){
      .countdown{font-size:.8rem; min-width:170px}
...
<body>
  <main class="card" role="status" aria-live="polite" aria-atomic="true">
    <div class="brand" aria-hidden="true">MINIWASH<span>.</span></div>

    <div class="badge" aria-hidden="true">
      <!-- scalable, crisp checkmark -->
      <div class="icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false" role="img">
          <circle cx="12" cy="12" r="11" fill="#ECF8EE"/>
          <path d="M7.5 12.6l2.6 2.6 6.4-6.4" stroke="#35612A" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </div>
    </div>
    ...
    <div class="countdown" aria-live="polite">
      Returning to store in <strong id="t" style="margin-left:.5rem">5</strong> seconds...
    </div>

    <!-- hidden helper for screen-readers with more descriptive status -->
    <p class="sr-only" id="sr-status">Payment confirmed. Will redirect to store in 5 seconds.</p>
  </main>

  <script>
    (async function () {
      // visual ready class for subtle motion
      try { document.body.classList.add('loaded'); } catch(e){}

      // preserve original behaviour: read sessionStorage and post if present
      try {
        const datos = JSON.parse(sessionStorage.getItem('miniwash_reservation'));
        if (datos) {
          // try navigator.sendBeacon first (fire-and-forget), fallback to fetch
          const url = '/api/save-payment';
          const payload = JSON.stringify(datos);
          const sent = (navigator && navigator.sendBeacon && navigator.sendBeacon(url, new Blob([payload], {type: 'application/json'})));
          if (!sent) {
            // fetch with timeout
            ...
            }).catch(()=>{ /* ignore network errors — we keep UX smooth */ })
        ...
        console.warn && console.warn('save-payment failed', err);
      }

      // countdown logic: robust and accessible
      (function countdown(startSeconds, onFinish){
        let s = Number(startSeconds);
        ...
        const tick = () => {
          ...
          if (s <= 0) {
            el.textContent = '0';
            if (sr) sr.textContent = 'Redirecting now.';
            clearInterval(timer);
        ...
        setTimeout(() => {
          clearInterval(timer);
          try { onFinish(); } catch(e){}
        }, (startSeconds + 1) * 1000 + 500); // slightly after expected end
      })(5, function(){
